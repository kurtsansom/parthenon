...
//========================================================================================
#ifndef COORDINATES_CYLINDRICAL_HPP_
#define COORDINATES_CYLINDRICAL_HPP_
...
class Cylindrical {
 public:
  Cylindrical() = default;
  Cylindrical(const RegionSize &rs, ParameterInput *pin) {
    // stays the same for r, and z
  ...
  Cylindrical(const Cylindrical &src, int coarsen)
  ...
  KOKKOS_FORCEINLINE_FUNCTION Real EdgeLength(const int dir, Args... args) const {
    if dir == 2:
      return Dx(dir) * x1f(args);
    else:
      return Dx(dir);
  }

  KOKKOS_FORCEINLINE_FUNCTION
  Real x1v(const int i) const { 
    ...
    return (TWO_3RD)*(std::pow(x1f(i+1), 3) - std::pow(x1f(i), 3)) /
 ...
 private:
  ...
  Real cell_volume_;
  constexpr static const char *name_ = "Cylindrical";
...
#endif // COORDINATES_CYLINDRICAL_HPP_
